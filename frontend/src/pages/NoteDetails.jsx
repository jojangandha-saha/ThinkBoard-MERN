...
import { LoaderIcon, ArrowLeftIcon, Trash2Icon, Heading1 } from 'lucide-react'
...
const handleDelete = async ()=>{
 ...
 console.log("Error in deeleting note", err)
 toast.error("Error in deleting note")
  ...
  const handleSave = async ()=>{
   ...
   setSaving(true)
   try{
    //pass the note object itself
    await api.put(`/notes/${id}`, note)
    toast.success("Note updated successfully")
    navigate("/")
   }catch(err){
    console.log("Error in saving note", err)
    toast.error('Error in saving the note')
   }finally{
    setSaving(false)
   }
  }
